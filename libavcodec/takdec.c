...
typedef struct TAKDecContext {
    ...
    int32_t        *decoded;                    ///< decoded samples

    int8_t         lpc_mode[MAX_CHANNELS];
    ...
    MCDParam       mcdparams[MAX_CHANNELS];     ///< multichannel decorrelation parameters

    int            wlength;
    int8_t         coding_mode[128];
    int            uval;
    int            rval;
    int32_t        filter[MAX_PREDICTORS];
...
static int decode_segment(TAKDecContext *s, int8_t value, int32_t *ptr, int len)
{
...
static int decode_coeffs(TAKDecContext *s, int32_t *ptr, int length)
{
    GetBitContext *gb = &s->gb;
    int i, v, ret = 0;
    ...
    if (get_bits1(gb)) {
        ...
        while (i < s->wlength) {
            ...
            if ((ret = decode_segment(s, v, ptr, len)) < 0)
                return ret;
            ptr += len;
        }
    } else {
        ...
        if (v > FF_ARRAY_ELEMS(xcodes))
            return AVERROR_INVALIDDATA;
        if ((ret = decode_segment(s, v, ptr, length)) < 0)
            return ret;
    }
...
static int decode_subframe(TAKDecContext *s, int32_t *ptr, int subframe_size, int prev_subframe_size)
{
    ...
    if (get_bits1(gb)) {
        ...
        s->filter[0] = s->predictors[0] << 6;
        for (i = 1; i < s->filter_order; i++) {
            int32_t *p1 = s->filter;
            int32_t *p2 = &s->filter[i - 1];
        ...
        for (i = 0, j = s->filter_order - 1; i < s->filter_order / 2; i++, j--) {
            tmp = y + s->filter[j];
            s->filter[j] = -(x & -(y + s->filter[i] >> 31) |
                            (y + s->filter[i]) >> (s->ared + 5));
            s->filter[i] = -(x & -(tmp >> 31) | (tmp >> s->ared + 5));
        }

        if ((ret = decode_coeffs(s, &ptr[s->filter_order], subframe_size - s->filter_order)) < 0)
            return ret;
...
static int decode_channel(TAKDecContext *s, int chan)
{
    ...
    GetBitContext  *gb = &s->gb;
    int32_t *p = s->decoded + chan * s->nb_samples;
    int i = 0, ret, prev = 0;
    ...
    *p++ = get_code(gb, avctx->bits_per_coded_sample - s->sample_shift[chan]);
    ...
    for (i = 0; i < s->nb_subframes; i++) {
        if ((ret = decode_subframe(s, p, s->subframe_len[i], prev)) < 0)
            return ret;
        p   += s->subframe_len[i];
...
{
    GetBitContext  *gb = &s->gb;
    uint32_t *p1 = &s->decoded[c1 * s->nb_samples + 1];
    uint32_t *p2 = &s->decoded[c2 * s->nb_samples + 1];
    int a, b, i, j, x, tmp;
    ...
    case 7:
        ...
        p1 += a;
        x = FF_ARRAY_ELEMS(s->residues) - s->nb_dcoeffs;
        for (; b > 0; b -= tmp) {
            tmp = FFMIN(b, x);

            for (i = 0; i < tmp; i++)
                s->residues[s->nb_dcoeffs + i] = *p2++ >> s->dshift;
            ...
            for (i = 0; i < tmp; i++) {
                ...
                for (j = 0; j < s->nb_dcoeffs; j += 8) {
                    v += s->residues[i + j + 7] * s->dcoeffs[j + 7] +
                         s->residues[i + j + 6] * s->dcoeffs[j + 6] +
                         s->residues[i + j + 5] * s->dcoeffs[j + 5] +
                         s->residues[i + j + 4] * s->dcoeffs[j + 4] +
                         s->residues[i + j + 3] * s->dcoeffs[j + 3] +
                         s->residues[i + j + 2] * s->dcoeffs[j + 2] +
                         s->residues[i + j + 1] * s->dcoeffs[j + 1] +
                         s->residues[i + j    ] * s->dcoeffs[j    ];
                }
            ...
            memcpy(s->residues, &s->residues[tmp], 2 * s->nb_dcoeffs);
        }
        break;
    }
                            ...
                            int *got_frame_ptr, AVPacket *pkt)
{
    ...
    if (avctx->bits_per_coded_sample <= 16) {
        ...
        s->decoded = s->decode_buffer;
    } else {
        s->decoded = (int32_t *)s->frame.data[0];
    }
