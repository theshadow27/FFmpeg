...
int main(int argc, char **argv) {
    int i=0, j, k, len, pid, afc, is_start, cc, packetnum = 0;
    int in_video = 0;
    ...
    for (i=0; i<len; i+=188, packetnum++) {
        ...
        for(j=0; 1; j++) {
            ...
            for (k=1; k<188; k++) {
                possible += popc8(headers[j].mask[k]); // TODO this should be precomputed... 
                if (headers[j].mask[k]) // count bits that are the same 
                    score += popc8(   headers[j].mask[k] &~ (buf[i+k] ^ headers[j].data[k])   ); // xnor
                else if (!headers[j].data[k])
                    break;  // zero header and mask, termination condition
            }
            
            double prob = score/(double)(possible);         
